...
class Mobilia(db.Model):
    ...
    def __str__(self):
        s = f'{self.nome},{self.id},{self.funcao},{self.material}'
        if self.quarto:
            s+=f' ,em{str(self.quarto)}'
        return s
...
if __name__ == "__main__": # teste das classes
    ...
    m1 = Mobilia(nome="Escrivaninha", funcao="Mesa para escrever", 
                    material="Madeira de carvalho", quarto=q1)
